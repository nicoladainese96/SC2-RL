...
from RelationalModule.AC_networks import SpatialActor, SpatialCritic #custom module
...
class MoveToBeaconSpatialA2C():
    ...
    * Possible to use twin networks for the critic and the critic target for improved 
    ...
    def __init__(self, action_space, n_layers, linear_size, in_channels, n_channels,
                 env, lr, gamma, TD=True, twin=True, tau = 0.3, H=1e-3, n_steps = 20, 
                 device='cpu', actor_lr=None, critic_lr=None, radam=False, **net_args):
        """
        Parameters
        ----------
        action_space: int
        ...
        **net_args: dict (optional)
        ...
        self.actor = SpatialActor(action_space, env, n_layers, linear_size, in_channels, n_channels, **net_args)
        self.critic = SpatialCritic(n_layers, linear_size, in_channels, n_channels, twin, **net_args)
        
        if self.TD:
            self.critic_trg = SpatialCritic(n_layers, linear_size, in_channels, n_channels, twin, target=True, **net_args)
    ...
    def step(self, obs, return_log=False):
        ...
        log_probs, spatial_features, nonspatial_features = self.actor(state, available_actions)
        if debug: 
    ...
    def get_arguments(self, spatial_features, nonspatial_features, action_id):
        ...
        if len(list_of_args) != 0:
            ...
            for arg in list_of_args:
                size = self.actor.all_arguments[arg.id].sizes
                if len(size) == 1:
                    arg_sampled, log_prob, _ = self.actor.sample_param(nonspatial_features, arg.name)
                else:
                    arg_sampled, log_prob, _ = self.actor.sample_param(spatial_features, arg.name)
